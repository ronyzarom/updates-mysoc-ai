...
SOURCE=ROOT/'deploy/observer-update-v1'
...
protocol=load('observer_update_protocol','protocol.py')
saved=sys.modules.get('protocol');sys.modules['protocol']=protocol
adapter=load('observer_update_adapter','adapter.py')
...
def binding():
    ...
    return dict(protocol=protocol.PROTOCOL,operation_id=str(uuid.uuid4()),machine_id='c'*32,
        installation_id='observer-1',updater_instance_id='updater-1',server_type='observer-unlinked',
    ...
    def setUp(self):
        ...
        def health(b,which):
            ...
            return dict(version=b[which]['version'],binary_sha256=b[which]['binary_sha256'],installation_id=b['installation_id'],updater_id=b['updater_instance_id'],
                installation_state='installed-unlinked',management_ready=True,pod_ready=False,authority_enabled=False,processing_enabled=False,ui_closed=which=='target' or self.capable,operator_login_usable=False)
