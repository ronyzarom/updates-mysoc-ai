...
import time
import unittest
from scripts.tests.test_observer_update_adapter import binding,protocol,SOURCE,load

saved=sys.modules.get('protocol');sys.modules['protocol']=protocol
worker=load('observer_update_worker','worker.py')
...
class TransportTests(unittest.TestCase):
    ...
    def run_worker(self,code,timeout=3):
        (self.bundle/'updater/observer_unlinked_update.py').write_text(code)
