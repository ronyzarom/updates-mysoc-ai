...
summary={'evidence_tier':tier,'adapter_kind':plan.get('adapter_kind'),'driver_sha256':digest(args.driver),'plan_sha256':digest(args.plan),'deployment_qualified':False,'scenarios':[]}
try:
 for index,scenario in enumerate(plan['scenarios']):
    ...
    try:
      ...
      for number,step in enumerate(scenario['runs']):
        ...
        passed=(result==0)==(step['expected']=='success');entry={'returncode':result,'expected':step['expected'],'matched':passed,'config_sha256':digest(config),'journals':{}}
        for name in ('operation.json','recovery-v2.json','next-operation.json'):
        ...
        for archived in (pathlib.Path(c['directory'])/'archive').glob('*/*.json'):
          if archived.is_symlink() or archived.name not in ('operation.json','recovery-v2.json','transition.json'):continue
