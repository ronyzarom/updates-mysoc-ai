...
if os.geteuid()!=0 or os.environ.get('NORMAL_CLEAN_FIXTURE')!='1':
    ...
    def do_GET(self):
        ...
        if path=='/artifact':
            ...
            self.reply(dict(update_available=not records['reports'],latest_version=release['version'],download_url='/artifact',
                sha256=release['sha256'],signature=release['signature'],channel=release['channel'],update_group='alpha'))
        else:self.send_error(404)
    def do_POST(self):
        value=json.loads(self.rfile.read(int(self.headers['Content-Length'])))
        path=urlparse(self.path).path
        if path=='/api/v1/updates/siemcore/check':
            self.reply(dict(update_available=not records['reports'],latest_version=release['version'],download_url='/artifact',sha256=release['sha256'],signature=release['signature'],channel=release['channel'],update_group='alpha'))
            return
...
tls.load_cert_chain(str(root/'credentials/server.crt'),str(root/'credentials/server.key'))
...
command=[str(kit/'install.sh'),'--clean','--license-key','disposable-fixture',
    '--parent-url','https://'+'normal.fixture'+':'+str(server.server_port),
    ...
    '--signing-key',release['public_key'],'--server-type','normal',
...
try:
    ...
    assert records['heartbeats'][-1]['installation']==dict(kind='normal')
    ...
    assert 'independent_node_bootstrap: true' not in config
    ...
    health=json.loads(subprocess.check_output(['curl','--fail','--silent','https://'+'normal.fixture'+'/health/live'],text=True))
    ...
    receipt=dict(first_install_by_kit=True,kit_version='1.16.1.33-r1',architecture='amd64-emulated',
    ...
    print('PASS: literal kit installer, systemd relay, signed first apply, restart/identity/health; Normal')
finally:
