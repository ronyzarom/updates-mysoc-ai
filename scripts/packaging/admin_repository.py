...
import argparse, hashlib, json, pathlib, shutil, subprocess, tarfile, html
p=argparse.ArgumentParser()
p.add_argument('--binary',required=True);p.add_argument('--receipt',required=True)
p.add_argument('--provisioning-kit',required=True);p.add_argument('--output',required=True)
...
r=json.loads(pathlib.Path(a.receipt).read_text());version=r['version'];revision=version+'-r1'
...
for tier,name in [('mysoc','mysoc-updater'),('siemcore','siemcore-cascade-updater')]:
 ...
 if tier=='siemcore':
  with tarfile.open(a.provisioning_kit) as tf:
   base='siemcore-updater-kit-1.16.1.23/'
   sums={line.split()[1].removeprefix('./'):line.split()[0] for line in tf.extractfile(base+'SHA256SUMS').read().decode().splitlines()}
   for item in ['greenfield-hook.py','recovery/artifact.py','recovery/recovery.py','recovery/supervise.py','PROVISIONING_COMMIT']:
    data=tf.extractfile(base+item).read()
    if item!='PROVISIONING_COMMIT':assert hashlib.sha256(data).hexdigest()==sums[item]
    dst=kit/item;dst.parent.mkdir(exist_ok=True);dst.write_bytes(data)
 (kit/'docs').mkdir()
 for doc in ['SIEMCORE-INSTALLATION-ROLES.md','UPDATE-ENTRYPOINT-CONTRACT.md','RELAY-DEPLOYMENT.md','UPDATER-GUIDELINES.md']:
