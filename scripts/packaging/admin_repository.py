...
for tier,name in [('mysoc','mysoc-updater'),('siemcore','siemcore-cascade-updater')]:
 ...
 (kit/'PACKAGE.json').write_text(json.dumps({'package_revision':revision,'architecture':'linux-amd64','template_commit':source,'updater_version':version,'updater_sha256':r['sha256'],'roles':['standalone','pod-a','pod-b','witness','observer-unlinked'] if tier=='siemcore' else ['mysoc-relay']},indent=2)+'\n')
