...
func TestIndependentObserverRetainedRetry(t *testing.T) {
	for _, failure := range []string{"apply", "health"} {
		t.Run(failure, func(t *testing.T) {
			...
			if s.applyIndependentObserver(context.Background(), update) == nil {
			...
			os.Remove(marker)
			if err := s.applyIndependentObserver(context.Background(), update); err != nil {
				t.Fatal(err)
			}
			...
			if s.applyIndependentObserver(context.Background(), changed) == nil {
...
func TestIndependentObserverFailureNeverClaimsRollback(t *testing.T) {
	...
	cfg.Products = []ProductConfig{{Name: "siemcore", ServerType: "observer-unlinked"}}
	e := &recordingExecutor{}
