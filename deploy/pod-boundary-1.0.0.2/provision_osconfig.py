...
"""
import base64,fcntl,hashlib,json,os,stat,subprocess,tarfile,urllib.request
...
def verify_package(base):
 ...
 if receipt['version']!=VERSION or receipt['public_key']!=KEY:raise ValueError('wrong signing identity')
 archive=base/('pod-boundary-'+VERSION+'.tar.gz')
 if digest(archive)!=receipt['sha256']:raise ValueError('package checksum mismatch')
 (base/'key.der').write_bytes(bytes.fromhex('302a300506032b6570032100'+KEY))
 (base/'sig').write_bytes(base64.b64decode(receipt['signature'],validate=True))
 ...
 with tarfile.open(archive) as tar:
  for member in tar:
   ...
   if target.is_symlink():raise ValueError('unsafe extraction target')
   target.write_bytes(tar.extractfile(member).read());target.chmod(0o600)
