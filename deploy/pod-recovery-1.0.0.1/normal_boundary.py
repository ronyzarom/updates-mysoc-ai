...
INSTALLED = Path('/usr/local/lib/siemcore-pod-boundary/1.0.0.3/boundary.py')
...
class Boundary:
    ...
    def invoke(self, state, ref, action, compensation=False):
        with self.bundle(ref, state['public_key']) as bundle:
            ...
            try:
                ...
                # Deliberately exclude product exception text, stderr, paths and env.
                failure = {'schema_version': 1, 'transaction_id': ref['archive'].split('/')[0],
                           'boundary_version': '1.0.0.3', 'phase': 'compensate' if compensation else action,
                           'product': 'siemcore', 'version': ref['version'], 'sha256': ref['sha256'],
                           'unit': unit, 'reason_code': 'supervised_phase_failed'}
                state['phase_failures'] = (state.get('phase_failures', []) + [failure])[-3:]
                self.save(state, state['phase'])
                raise
    ...
    def dispatch(self, policy, application, phase):
        recovery = Path('/var/lib/updates-pod-recovery/transaction.json')
        # Fail closed even if malformed. Parent death must never reopen ordinary
        # updates while a supervised recovery worker or staged runtime exists.
        if os.path.lexists(recovery):
            raise RuntimeError('dedicated pod recovery requires explicit handoff; normal update refused')
        ...
        if state and state['phase'] == 'preflight-refused':
            from reconcile import validate_terminal
            validate_terminal(self, state, policy, application)
            if phase != 'apply':
                raise RuntimeError('preflight refused: no application or rollback success to report')
        ...
        if phase=='apply':
            if state and state['phase'] not in ('healthy','rolled-back','preflight-refused'):
