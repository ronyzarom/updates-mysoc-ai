...
"""
import base64,hashlib,json,os,pwd,subprocess,sys,urllib.request
...
def main():
 ...
 try:
  statepath=root/'state.json';state=statepath.read_bytes();config=Path('/etc/siemcore-cascade-updater/config.yaml').read_bytes()
  ...
  owner=pwd.getpwnam(service);destination=layout/'releases'/p['release']['version'];destination.mkdir(mode=0o755,exist_ok=True)
  ...
  receipt.write_text(json.dumps(expected,sort_keys=True));receipt.chmod(0o600)
