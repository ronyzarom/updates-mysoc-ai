...
POLICY=Path('/etc/siemcore-cascade-updater/node-update-policy.json')
LOCK=Path('/var/lib/siemcore-greenfield/hook.lock')


def admission():
    policy=private_json(POLICY)
    if set(policy)!={'protocol','enabled','component_manifest_sha256','bootstrap_policy_sha256','bootstrap_binary_sha256','product_channel','allowed_transition'} or policy['protocol']!=PROTOCOL or policy['enabled'] is not True:
        raise ValueError('Node update component is not enabled')
    ...
    link=Path('/opt/siemcore-node-unlinked-'+app['node_id'])/'link.json'
    links=[link,Path('/etc/siemcore-pod-node/link.json'),Path('/etc/siemcore-pod-controller/controller.json'),Path('/var/lib/siemcore-greenfield/pod-runtime.json')]
    if any(path.exists() or path.is_symlink() for path in links):raise ValueError('linked Node refused')
    app=private_json(APPLICATION)
    if (app.get('schema')!=5 or app.get('topology')!='node-unlinked' or
    ...
    if len(types)!=1 or types[0].strip('"')!='pod-node' or channels!=[policy['product_channel']]:
...
def create_binding(host,operation,target,previous_binding=None):
    ...
    result=dict(protocol=PROTOCOL,operation_id=operation,machine_id=app['machine_id'],installation_id=app['installation_id'],updater_instance_id=app['updater_instance_id'],server_type='pod-node',node_id=app['node_id'],bootstrap_policy_sha256=hashlib.sha256(json.dumps(app,sort_keys=True).encode()).hexdigest(),predecessor=predecessor,target=desired,signing_public_key_sha256=hashlib.sha256(host.public_key).hexdigest(),ui_protection_required=True)
...
def main():
    ...
    with os.fdopen(fd,'a') as lock:
        ...
        transition=policy['allowed_transition']
        if transition != {role:{k:binding[role][k] for k in ('version','artifact_sha256','artifact_signature')} for role in ('predecessor','target')}:
            raise ValueError('transition outside signed component authorization')
...
if __name__=='__main__':
    try:main()
        ...
        print('Node adapter refused or retained uncertain operation; inspect protected journal',file=sys.stderr)
        sys.exit(1)
