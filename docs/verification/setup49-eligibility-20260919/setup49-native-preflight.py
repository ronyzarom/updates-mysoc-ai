"""Exact setup49 native preflight; never invokes product apply or reads DB data."""
import argparse,copy,fcntl,hashlib,importlib.util,json,os,pathlib,socket,stat,subprocess,sys,tempfile,time
P=pathlib.Path
SHA='88d4bbde2b5e9ccdbb572285f03b5f1daac1283880c575186f9be321cbdf8b70'
...
require(r['version']=='3.3.152.49' and r['sha256']==SHA,'wrong candidate')
entry=dict(artifact='/var/lib/siemcore-cascade-updater/artifacts/siemcore-3.3.152.49.artifact',sha256=SHA,signature=r['signature'])
legacy=socket.gethostname()=='ip-172-26-13-54'
...
if legacy:
 ...
 require(p['policy_revision']==8 and '3.3.152.49' not in p['releases'],'unexpected policy')
 p['policy_revision']=9;p['allowed_transitions'].append(['3.3.152.39','3.3.152.49']);p['releases']['3.3.152.49']=entry
 p['expected_mounts_by_version']['3.3.152.49']=copy.deepcopy(old['expected_mounts_by_version']['3.3.152.39'])
 ...
 p=dict(schema=1,hostname=socket.gethostname(),install_dir=str(hook.APP_DIR),cascade_root='/opt/siemcore-cascade',journal_dir='/var/lib/siemcore-recovery',cluster_id=marker['cluster_id'],node_id='app-a',project_name='siemcore-app-app-a',containers={'siemcore':'siemcore-app-a','siemcore-archiver':'siemcore-archiver-app-a'},allow_dynamic_archiver_debug_port=True,health_url='http://127.0.0.1:8443/health',dashboard_dir=None,public_key=bootstrap['public_key'],allowed_transitions=[[before,'3.3.152.49']],apply_timeout=480,health_timeout=120,releases={before:previous,'3.3.152.49':entry})
...
with open(store/'lock','a') as lock:
 ...
 with tempfile.TemporaryDirectory(dir=store,prefix='setup49-preflight-') as temp:
  tx=recovery.Lifecycle(dict(p,journal_dir=temp)).snapshot('3.3.152.49')
