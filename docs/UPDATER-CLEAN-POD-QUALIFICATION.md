...
A/B relay TLS: add --relay-cert-file /etc/ssl/siemcore/fullchain.pem --relay-key-file /etc/ssl/siemcore/privkey.pem. Installer verifies certificate validity and key match, copies the pair as root:siemcore-cascade-updater mode0640 under the mode0750 updater config directory, and sets explicit relay TLS paths before service start. Original SSL.com files remain untouched. Witness may omit these flags and retain private relay TLS.
