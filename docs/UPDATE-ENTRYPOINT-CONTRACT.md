...
the missing directory and exit non-zero.
...
2. **Bounded** — must finish within the operator's `command_timeout`
   (300s on current hosts, includes any health polling you do internally).
3. **Never touch host-side secrets** — `.env` and equivalent host
   configuration are outside the artifact's authority. Write deploy
   provenance to your own files (e.g. `DEPLOY_INFO.json`).
4. **Database migrations are explicit** — either the entrypoint applies and
   records them, or it must exit non-zero when unapplied migrations are
   detected. Silently running code against a mismatched schema is the one
...
- siemcore: the app repo's `update.sh` (v3 compose contract).
